class AddMissingObservations2
  ...
  if ARGV.empty?
    puts "supply an experiment id"
    exit
  end
  
  the_exp = Experiment.find ARGV.first
  conds = the_exp.conditions
...
#  exit if true
  ...
  Observation.transaction do
    begin            
      ...
      for cond in conds
        Condition.connection.execute("delete from observations where condition_id = #{cond.id}")
      end
